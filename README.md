...
- **预览**：
  - 视频 / 音频 / 图片：流式加载，URL 直接交给媒体元素按 Range 边下边播，无需等待整文件下载（分片合并文件仍需完整拉取）
  - 文本：纯文本或代码着色（按扩展名自动识别 JavaScript / JSON / Python / C 等，可手动切换），长行不换行走水平滚动
  - Markdown：默认渲染预览（标题/列表/引用/代码块/链接/图片等），可切回纯文本查看原文
  - ZIP：利用中央目录位于文件尾部的特性，HTTP Range 仅取尾部与目录即列出文件清单；点选条目按需取范围解压（浏览器内置 DecompressionStream，无外部依赖），图片/音视频/文本直接预览，超大文件提供下载；代理不支持 Range 时自动回退为整包下载解析；ZIP64 与非常规压缩方式给出明确提示
