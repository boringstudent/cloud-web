...
- **文件浏览**：文件夹层级导航、面包屑、文件/文件夹大小显示（文件夹大小由 git tree 递归统计）；标题右侧显示全仓**总用量**（数据量）
...
- **上传**：
  ...
  - 失败自动清理残留分片
- **下载**：
  - 单文件下载先拉取为 Blob 再用 ObjectURL 保存（跨域时 `download` 属性会被忽略，直接跳转预览的问题由此规避），底部**进度条卡片**实时显示百分比、已下载大小与速率，可点 ✕ 中止
  - 分片文件 6 并发合并下载，同样驱动进度条卡片
  - 复选批量下载：串行逐个下载（完成/失败一个再下一个，避免浏览器拦截并发自动下载并互相抢占带宽），进度条按字节汇总总进度，首个文件提示浏览器多文件拦截的处理方式，可随时停止并立即中止当前传输
  - **文件夹下载**：右键文件夹即可下载，经 git tree 收集全部文件（分片文件按组归并还原）后复用串行下载管线，带总进度条与停止按钮
...
- **预览**：
  ...
  - 图片查看器：工具栏 放大/缩小/适应/1:1，滚轮缩放、双击切换适应与实际大小、放大后拖拽平移；当前目录多图时显示**缩略图条**与前后切换按钮（← → 方向键同样可切换），切换走浏览器缓存即时呈现
