...
- **预览**：
  - 视频 / 音频：流式加载，URL 直接交给媒体元素按 Range 边下边播，无需等待整文件下载；打开时探测拉取文件头部 1MB 测得真实下载速度，实时显示速度与已缓冲百分比，完成后保留 3 秒（分片视频仍需完整拉取；分片音频每收完一段分片即可播放已下载部分，合并完成后自动切换完整文件并续播）
  - 图片：fetch 分块流式读取，部分数据渐进渲染边下边显示，角落与加载文案实时显示下载速度与百分比；关闭预览即中断下载，不浪费带宽
